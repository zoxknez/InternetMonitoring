...
public sealed class LinuxAtomicProvisioningAndRecoveryTests
{
    ...
    public void Atomic_Publisher_Handles_Race_Collision_With_Winner()
    {
        ...
        // Hook simulates that another process wins rename race immediately before our rename
    ...
    [Fact]
    public void Atomic_Publisher_Parent_Fsync_Failure_Aborts_Publication()
    {
        var mock = new MockPosixStorageApi { FailParentDirFsync = true };
        mock.AddEntry("/var/lib/internet-evidence-monitor/keys", isDir: true, isSymlink: false, mode: 0x1C0, uid: 1000, gid: 1000);
        int keysFd = mock.Open("/var/lib/internet-evidence-monitor/keys", 0, 0);
        ...
        var policy = LinuxStorageOwnershipPolicy.CreateSystem(uid: 1000, gid: 1000);
        var content = "SAMPLE_CONTENT"u8.ToArray();

        var ex = Assert.Throws<InvalidOperationException>(() =>
            LinuxAtomicFilePublisher.PublishAtomically(mock, keysFd, "layout.json", content, LinuxPosixStorageConstants.Mode0600, policy));

        Assert.Contains("durability", ex.Message);
    }

    // ==========================================
    // 8D-D: Crash-Point State Machine (CP-01 to CP-09)
    // ==========================================
    ...
    public void Crash_Point_CP03_Partial_Write_Leaves_No_Final_File()
    {
        var mock = new MockPosixStorageApi
        {
            FailWrite = true // Crash during write
        };
        mock.AddEntry("/var/lib/internet-evidence-monitor/keys", isDir: true, isSymlink: false, mode: 0x1C0, uid: 1000, gid: 1000);
        int keysFd = mock.Open("/var/lib/internet-evidence-monitor/keys", 0, 0);
    ...
    public void Crash_Point_CP06_Pre_Rename_Failure_Leaves_No_Final_File()
    {
        var mock = new MockPosixStorageApi
        {
            FailRenameAt2Errno = LinuxPosixStorageConstants.EIO
        };
        mock.AddEntry("/var/lib/internet-evidence-monitor/keys", isDir: true, isSymlink: false, mode: 0x1C0, uid: 1000, gid: 1000);
        int keysFd = mock.Open("/var/lib/internet-evidence-monitor/keys", 0, 0);
    ...
    // ==========================================
    // 8D-E: Stale Temp Recovery
    // ==========================================

    [Fact]
    public void Cleanup_Stale_Temp_Files_Safely_Deletes_App_Owned_Temp()
    {
        var mock = new MockPosixStorageApi();
        mock.AddEntry("/var/lib/internet-evidence-monitor/keys", isDir: true, isSymlink: false, mode: 0x1C0, uid: 1000, gid: 1000);
        mock.AddEntry("/var/lib/internet-evidence-monitor/keys/.tmp.key.12345abc", isDir: false, isSymlink: false, mode: 0x180, uid: 1000, gid: 1000, content: "ORPHAN"u8.ToArray());
        int keysFd = mock.Open("/var/lib/internet-evidence-monitor/keys", 0, 0);

        var policy = LinuxStorageOwnershipPolicy.CreateSystem(uid: 1000, gid: 1000);

        bool cleaned = LinuxAtomicFilePublisher.CleanupStaleTempFile(mock, keysFd, ".tmp.key.12345abc", policy);
        Assert.True(cleaned);
        Assert.False(mock.TryGetEntry("/var/lib/internet-evidence-monitor/keys/.tmp.key.12345abc", out _));
    }
    ...
    public void Cleanup_Stale_Temp_Files_NEVER_Deletes_Foreign_Owned_File()
    {
        var mock = new MockPosixStorageApi();
        mock.AddEntry("/var/lib/internet-evidence-monitor/keys", isDir: true, isSymlink: false, mode: 0x1C0, uid: 1000, gid: 1000);
        mock.AddEntry("/var/lib/internet-evidence-monitor/keys/.tmp.key.foreign", isDir: false, isSymlink: false, mode: 0x180, uid: 0, gid: 0); // Root owned!
        int keysFd = mock.Open("/var/lib/internet-evidence-monitor/keys", 0, 0);

        var policy = LinuxStorageOwnershipPolicy.CreateSystem(uid: 1000, gid: 1000);

        bool cleaned = LinuxAtomicFilePublisher.CleanupStaleTempFile(mock, keysFd, ".tmp.key.foreign", policy);
        Assert.False(cleaned);
        Assert.True(mock.TryGetEntry("/var/lib/internet-evidence-monitor/keys/.tmp.key.foreign", out _)); // NOT deleted!
    }
    ...
    public void Cleanup_Stale_Temp_Files_NEVER_Follows_Or_Deletes_Symlink()
    {
        var mock = new MockPosixStorageApi();
        mock.AddEntry("/var/lib/internet-evidence-monitor/keys", isDir: true, isSymlink: false, mode: 0x1C0, uid: 1000, gid: 1000);
        mock.AddEntry("/var/lib/internet-evidence-monitor/keys/.tmp.key.symlink", isDir: false, isSymlink: true, mode: 0x180, uid: 1000, gid: 1000); // Symlink!
        int keysFd = mock.Open("/var/lib/internet-evidence-monitor/keys", 0, 0);

        var policy = LinuxStorageOwnershipPolicy.CreateSystem(uid: 1000, gid: 1000);

        bool cleaned = LinuxAtomicFilePublisher.CleanupStaleTempFile(mock, keysFd, ".tmp.key.symlink", policy);
        Assert.False(cleaned);
    }
