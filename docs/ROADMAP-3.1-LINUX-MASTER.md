...
> **Status (2026-09-10):** 3.1-0..3.1-12 implementirano, 3.1-13 u toku. 3.1-10 Avalonia shell i crash-isolation gate, 3.1-11 Linux pakovanje/SBOM i kompletan 3.1-12 cross-platform parity su na `main`; UI polish (Wayland/HiDPI/a11y/multi-monitor) ostaje za završni release rez. Parity pokriva svih 26 validnih §25.12 scenarija i zaseban namerno nevalidan stronger-Linux-claim fixture. Debian lifecycle sada u čistom okruženju proverava install, stvarni version upgrade kada je prethodni paket prosleđen, reinstall i purge, uz byte-for-byte očuvanje sistemskih dokaza, signing identiteta i odvojenog portable XDG stanja; clean-package varijanta je deo redovnog Linux CI-ja. Linux GUI pri padu IPC veze eksplicitno prelazi u `ServiceUnavailable`, ostaje u system-service modu i ne pokreće portable fallback. Lane C sada zahteva i beleži isti system session ID kroz stop/start, normalan restart i crash + `Restart=on-failure`; preostaje izvršenje tog gate-a na Tier-A VM i poseban machine-reboot dokaz. 3.1-14..3.1-15 su planirani. **Dopuna (2026-09-11):** Lane C je prvi put stvarno izvršen (pre-flight u Ubuntu 24.04 kontejneru sa systemd PID 1; bez Tier-A autoriteta po §20.2/§31.C). Runner je od 3.1-11 objavljivao servis ravno u `${INSTALL_DIR}`, dok kanonski unit starta `${INSTALL_DIR}/service/...`, pa je gate 5 padao sa `203/EXEC` i sve dalje ostajalo `NOT_TESTED` — greška je preživela jer je runner proveravan samo sa `bash -n`. Posle ispravke layout-a gate frontier je 4→9 PASS, a gate 10 je otkrio da je `RestrictSUIDSGID=yes` nespojiv sa `openat2` (seccomp ne vidi `mode` u `struct open_how`, pa systemd blokira ceo syscall): servis se digne, javi `READY=1`, pa pada sa exit 3 na prvom `StartSession` i ulazi u restart petlju — bez ijednog snimljenog dokaza. Direktiva je sada `CONFIRMED_OFF`. Dodat je `scripts/ci/verify-linux-session-smoke.sh`, jer nijedna install-level provera ne pokreće sesiju i zato je ovaj kvar prošao kroz sve zelene gate-ove. Ispravljeni su i SBOM `GitCommit` (bio `unknown` van CI-ja) i SBOM koji je brojao `packages.lock.json` iz `bin/`/`obj/`, pa je isti commit davao različit dokument na različitim stablima. Posle ispravki gate 10 prolazi 5 od 6 pod-testova (outsider denial, GetServiceStatus, StartSession, spoof cross-user stop denial, vlasnikov StopSession); runner je usput prijavljivao pad proizvoda zato sto mu je nedostajao `jq` (koristi ga na 11 mesta, bez ikakve provere) — sada postoji eksplicitna provera alata. Otvoreno ostaje: posle pause+resume `FinalizeSession` vraca `SESSION_NOT_FOUND` dok `GetServiceStatus` i dalje imenuje sesiju (`SessionState.Interrupted`, `Resumed=false`); isto za vlasnika i za `iem-admin`, reprodukovano na isporucenom `.deb`-u. Nije krpljeno jer zavisi od namerene pause/resume semantike.
...
| `RestrictSUIDSGID=yes` | nema setuid/setgid binar | **totalan gubitak funkcije**: systemd pod ovom direktivom blokira `openat2` u celosti (seccomp ne može da pročita `mode` unutar `struct open_how`), a storage boundary svaki dokazni put razrešava kroz `openat2` | start prolazi, ali prvi `StartSession` ruši servis sa exit 3 i ulazi u restart petlju; provereno na `.deb` 3.1.0~alpha3 u Ubuntu 24.04 systemd PID 1 kontejneru | **CONFIRMED_OFF** — `RestrictSUIDSGID=no`. `NoNewPrivileges=yes` već pokriva eskalaciju preko setuid/setgid binara; servis piše isključivo u svoj `0700` StateDirectory pod `UMask=0077` |
