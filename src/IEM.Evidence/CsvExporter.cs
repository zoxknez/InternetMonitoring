...
public static class CsvExporter
{
    ...
    public static void WriteSummary(string path, SessionSnapshot session)
    {
        ...
        var lines = new List<string>
        {
            ...
            $"Izolovano iza rutera:   {session.UpstreamIncidents.Count()}",
            $"Nedostupnost iza rutera: {SerbianText.Duration(session.UpstreamDowntime)}",
            $"Lokalna nedostupnost:   {SerbianText.Duration(session.LocalDowntime)}",
            $"Najduži prekid iza rutera: {SerbianText.Duration(session.LongestUpstreamOutage)}",
            "",
